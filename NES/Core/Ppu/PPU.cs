    ...
    class PPU
    {
        ...
        void ReadSpriteScanline()
        {
            ...
            foreach (var sprite in line)
            {
                ...
                if (flipY && Control.SpriteSize > row)
        ...
        public void Poke(int addr, byte data)
        {
            ...
            switch (addr)
            {
                ...
                case 4:
                    ...
                    {
                        case 0:
                            ActSprite = new TileRow();
                            if (OamAddr == 0)
                            {
                                SpriteZero = ActSprite;
                                SpriteLines = new List<TileRow>[Screen.Height];
                            }
                            ActSprite.Y = data;
                            for (int row = ActSprite.Y; row < ActSprite.Y + Control.SpriteSize; row++)
                            {
                                if (row >= SpriteLines.Length) break;
                                if (SpriteLines[row] == null) SpriteLines[row] = new List<TileRow>();
                                SpriteLines[row].Add(ActSprite);
                            }
                            break;
                        case 1: ActSprite.ID = data; break;
                        case 2: ActSprite.Attribute = data; break;
                        case 3: ActSprite.X = data; break;
                    }
                    Oam[OamAddr] = data;
                    OamAddr++;
                    break;
