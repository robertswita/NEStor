...
namespace NES
{
    ...
    partial class CPU
    {
        ...
        byte ASL(int address, bool carry)
        {
            var mem = Read(address);
            Write(address, mem);
        ...
        public byte LSR(int address, bool carry)
        {
            var mem = Read(address);
            Write(address, mem);
        ...
        //public void DEC(int address) { DecMem(address); }
        ...
        //public void INC(int address) { IncMem(address); }
        ...
        void CMP(byte reg, byte mem)
        {
            ...
            //SetZeroNegativeFlags((byte)(reg - mem));
            Result = (byte)(reg - mem);
        }
        ...
        public void SLO(int address) { A |= ASL(address, false); A |= Result; }
        public void RLA(int address) { A &= ASL(address, Status.Carry); }
        public void SRE(int address) { A ^= LSR(address, false); }
        public void RRA(int address) { ADC(LSR(address, Status.Carry), 0x00); }
