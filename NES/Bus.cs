    ...
    class Bus
    {
        ...
        public void CpuWrite(int addr, byte data)
        {
            ...
            if (addr < 0x2000) // 2K Internal RAM
                               ...
                               //else
                InternalRAM[addr & 0x07FF] = OpenValue; // _mirror by masking first block range
            else if (addr <= 0x3FFF) // NES PPU registers
                PPU.CpuWrite(addr & 0x0007, OpenValue); // Mirrors of $2000-2007 (repeats every 8 bytes)
            else if (addr == 0x4014)
            {
                //PPU.OpenBus = data;
            ...
            else if (addr == 0x4017)
            {
                ...
                {
                    //CPU.IRQDelay = 1;
                    //CPU.IRQList.Add(CPU.IRQ);
                }
            }
            else if (addr >= 0x6000 && addr < 0x8000)
                WRAM[addr & 0x1FFF] = OpenValue;
            else if (addr >= 0x8000)
                Mapper.CpuMapWrite(addr, OpenValue);
        }
